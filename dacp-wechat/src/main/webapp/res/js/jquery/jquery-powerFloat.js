...
(function($) {
	$.fn.powerFloat = function(options,isTrigger) {
			...
			if(isTrigger){
				if (o.display && o.trigger && e.target === o.trigger.get(0)) {
					...
					if (!$(document).data("mouseupBind")) {
						$(document).bind("mouseup.powerFloat", function(e) {
							var flag = false;
							if (o.trigger) {
								...
								if (s.eventType === "click" && o.display && e.target != o.trigger.get(0) && !flag && !$(o.target).hasClass("uping")) {
									o.flagDisplay = false;	
									o.displayDetect();
								}
							}
				...
				default: {
					...
					o.targetGet();
					// 放置页面点击后显示的浮动内容隐掉
					$(document).unbind("mouseup.powerFloat").data("mouseupBind", false);
				}
				}
			}
		});
